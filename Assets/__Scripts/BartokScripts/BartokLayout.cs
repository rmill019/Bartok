...
public class BartokLayout : MonoBehaviour {
	...
	public void ReadLayout (string xmlText)
	{
		...
		for (int i = 0; i < slotsX.Count; i++)
		{
			...
			// This converts the numbertt of the layerID into a text layerName
			tSD.layerName = tSD.layerId.ToString();
			// The layers are used to make sure that the correct cards are on top of the others.
			// In Unity 2D,all of our assets are effectively at the same z depth, so sorting
			// layers are used to differentiate between them
			...
			case "drawpile":
				// The drawPile xstagger is read but not acutally used in Bartok
				tSD.stagger.x = float.Parse (slotsX [i].att ("xstagger"));
				drawPile = tSD;
				break;

			case "target":
				// The target card has a different layout from discardPile
				target = tSD;
				break;
