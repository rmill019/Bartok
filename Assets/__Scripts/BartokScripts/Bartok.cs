...
public class Bartok : MonoBehaviour {
	...
	void LayoutGame()
	{
		...
		{
			for (int j = 0; j < 4; j++)	// There are always 4 players hece the "j < 4" argument
			{
				tCB = Draw ();		// Draw a card
				// Stagger the draw time a bit. Remember order of operations
				tCB.timeStart = Time.time + drawTimeStagger * (i * 4 + j);
				// ^ By setting the timeStart before calling AddCard, we override the
				// automatic setting of timeStart in CardBartok.MoveTo();

				// Add the card to the player's hand. The modulus (%4) results in a number from 0 t0 3
	...
	// This callback is used by the last card to be dealth at the beginning
	...
	public bool CheckGameOver ()
	{
		...
		if (CURRENT_PLAYER.hand.Count == 0)
		{
			...
			if (CURRENT_PLAYER.type == PlayerType.human)
			{
				...
				AudioManager.S.GetComponent<AudioSource> ().clip = AudioManager.S.gameWon;
			}
			else
			{
				...
				AudioManager.S.GetComponent<AudioSource> ().clip = AudioManager.S.gameOver;
			}
