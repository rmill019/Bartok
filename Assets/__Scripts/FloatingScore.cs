...
public class FloatingScore : MonoBehaviour {
	...
	//The score prorperty sets both _score and scoreString
	public int score{
		get{ 
			return (_score);
		}
		set{ 
			_score = value;
			scoreString = _score.ToString ("N0"); //N0 adds commas to the num Search "C# Standard Numeric Format Strings" for ToString formats
	...
	public void FSCallback(FloatingScore fs){
		//When this callback is called by SendMessage, add the score from calling FloatingScore
		score += fs.score;
	}
		...
		if (u < 0) {	//If u<0, then we shouldn't move yet
			...
			if (u >= 1) { 	//If u>=1, we're done moving
				...
				if (reportFinishTo != null) {	//If there's a callback GameObject
					//Use SendMessage to call the FSCallback method with this as a the paramater
			...
			//Use Bezier curve to move this to the righ point
